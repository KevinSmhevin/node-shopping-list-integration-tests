...
describe("Recipes", function() {
  ...
  it('should list items & ingredients on GET', function() {
      return chai
      .request(app)
      .get("/recipes")
      .then(function(res) {
  ...
  it("should add an item with ingredients on POST", function() {
      const newItem = { name: "chocolate", ingredients: ["sugar", "cocoa"] };
      return chai
      .request(app)
      .post("/recipes")
      .send(newItem)
  ...
  it("should update ingredients on PUT", function() {
      const updateData = { name: "milkshake", ingredients: ["cocoa", "ice-cream"] };
      return (
          chai
          .request(app)
          .get("/recipes")
  ...
  it("should delete item and ingrededients on PUT", function() {
    return (
      chai
      .request(app)
      .get("/recipes")
      .then(function(res) {
        const deleteData = res.body[0].id;
        return chai
        .request(app)
        .delete(`/recipes/${deleteData}`)
    })
    .then(function(res) {
        expect(res).to.have.status(204);
    })
